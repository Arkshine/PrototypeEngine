...
{
	g_MetaLoader.SetGameDirectory( pEnginefuncs->pfnGetGameDirectory() );

	g_MetaLoader.Run( true );
...
{
	{
		...
		g_MetaLoader.SetGameDirectory( szGameDir );
	}

	g_MetaLoader.Run( false );
}
}
...
)
{
	if( dliNotify == dliNotePreLoadLibrary )
	{
		if( strcmp( pdli->szDll, "Tier1.dll" ) == 0 )
		{
			...
			if( !( *g_MetaLoader.GetGameDirLocal() ) )
				return nullptr;

			const int iResult = snprintf( szPath, sizeof( szPath ), "%s/%s/%s", g_MetaLoader.GetGameDirLocal(), filepaths::BIN_DIR, pdli->szDll );
